...
export default class KeyboardLayout extends KeyboardLayoutDesign {
  ...
  constructor(
    props?: ConstructorParameters<typeof FlexLayout>,
    pageName?: string
  ) {
    super(props);
  ...
  static init(textBoxes: Array<TextBox> | TextBox = []) {
    ...
    currentTextBoxes.forEach((textBox, index) => {
      if (textBox instanceof TextBox) {
        ...
        ThemeService.instance.addPage(
          createPageContext(keyboardLayout, `keyboardLayout${index}`),
...
function setDefaultActions(keyboardLayouts: Array<KeyboardLayout> = []) {
  keyboardLayouts.forEach((keyboardLayout, index) => {
    ...
    keyboardLayout.onUpImageClick = () =>
      keyboardLayouts[index - 1] &&
      keyboardLayouts[index - 1].textBox?.requestFocus();
    keyboardLayout.onDownImageClick = () =>
      keyboardLayouts[index + 1] &&
      keyboardLayouts[index + 1].textBox?.requestFocus();
    keyboardLayout.toggleDisabilityofUpImage(index === 0);
    keyboardLayout.toggleDisabilityofDownImage(
      index === keyboardLayouts.length - 1
    );
